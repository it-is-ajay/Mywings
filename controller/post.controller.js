...
import {Post,Like,Comment,Save} from "../model/association.js";
...
export const getAllPost = (request,response,next)=>{
    try {
        let data = Post.findAll({where:{userId:request.body.userId}})
...
export const getAllLikes = (request,response,next)=>{
    try {
        let data = Like.findAll({where:{userId:request.body.userId,postId:request.body.postId}})
        .then((result)=>{return response.status(200).json({message:"data found",result:result,status:true})})
    } catch (error) {
       console.log(error);
...
export const getSavedPost = (request,response,next)=>{
    try {
        let data = Save.findAll({where:{userId:request.body.userId,postId:request.body.postId}})
        .then((result)=>{return response.status(200).json({message:"data found",result:result,status:true})})
    } catch (error) {
       console.log(error);
       return response.status(500).json({error:"internal server error",status:false});   
...
export const getAllComments = (request,response,next)=>{
    try {
        let data = Comment.findAll({where:{userId:request.body.userId,postId:request.body.postId}})
        .then((result)=>{return response.status(200).json({message:"data found",result:result,status:true})})
    } catch (error) {
       console.log(error);
       return response.status(500).json({error:"internal server error",status:false});   
